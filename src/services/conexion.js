...
export async function IniciarSesionUsuario(username,password){
    ...
    try{
        ...
        if(!passwordCorrect){
            return { sucess: false, message: 'Contraseña incorrecta' };
        }
        if(!user.estado){
            return { sucess: false, message: 'Usuario inactivo' };
        }
        return { sucess: true, message: 'Usuario Valido', user: {username:user.username, fullname:`${user.nombres} ${user.apellidos}`}};
    }catch (error){
        console.log('Error al iniciar sesion');
        return { sucess: false, message: 'Error al iniciar sesion' };
    }finally{
        await client.end();
    }
}
